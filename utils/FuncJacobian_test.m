...
function [Jacobian] = FuncJacobian_test(xVector,PVector,Feature,K,ID1,ID2,nJacobian,state_vector,extrinsic)
...
for j=1:nRowNumu/2;
    ...
    if PID == Feature(FID,3);
        P_I_i = state_vector{PID}.p;
        P_C_i = state_vector{PID}.p + state_vector{PID}.R*extrinsic(1:3,4); % CAM_Position: State vector is in IMU frame but I need Camera frame for PBA
        Fj = PVector.Feature(3*FID-2:3*FID-1,1);      
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Xj = [sin(Fj(1))*cos(Fj(2));
              sin(Fj(2));
              cos(Fj(1))*cos(Fj(2))];        
        R_I_i = state_vector{PID}.R; 
        R_C_i = state_vector{PID}.R*extrinsic(1:3,1:3);     % CAM_rotation         
        x=K*R_C_i'*Xj;
    ...
    elseif PID == Feature(FID,4);
        P_I_i = state_vector{Feature(FID,3)}.p;
        P_C_i = state_vector{Feature(FID,3)}.p + state_vector{Feature(FID,3)}.R*extrinsic(1:3,4);
        P_I_i = state_vector{PID}.p;
        P_C_k = state_vector{PID}.p + state_vector{PID}.R*extrinsic(1:3,4);
        Fj = PVector.Feature(3*FID-2:3*FID,1);        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Xj = [sin(Fj(1))*cos(Fj(2));
            sin(Fj(2));
            cos(Fj(1))*cos(Fj(2))];
        R_I_i = state_vector{Feature(FID,3)}.R;
        R_C_i = state_vector{Feature(FID,3)}.R*extrinsic(1:3,1:3);
        R_I_k = state_vector{PID}.R; 
        R_C_k = state_vector{PID}.R*extrinsic(1:3,1:3);        
        Tik = P_C_k-P_C_i;
        Dik = norm(Tik);
        DotP = Xj'*Tik;
        ...
        P_I_i = state_vector{Feature(FID,3)}.p;
        P_C_i = state_vector{Feature(FID,3)}.p + state_vector{Feature(FID,3)}.R*extrinsic(1:3,4);
        P_I_k = state_vector{Feature(FID,4)}.p;
        P_C_k = state_vector{Feature(FID,4)}.p + state_vector{Feature(FID,4)}.R*extrinsic(1:3,4);
        P_I_l = state_vector{PID}.p;
        P_C_l = state_vector{PID}.p + state_vector{PID}.R*extrinsic(1:3,4);
        Fj = PVector.Feature(3*FID-2:3*FID,1);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Xj = [sin(Fj(1))*cos(Fj(2));
              sin(Fj(2));
        ...
        R_I_i = state_vector{Feature(FID,3)}.R;
        R_C_i = state_vector{Feature(FID,3)}.R*extrinsic(1:3,1:3);
        R_I_k = state_vector{Feature(FID,4)}.R;
        R_C_k = state_vector{Feature(FID,4)}.R*extrinsic(1:3,1:3);
        R_I_l = state_vector{PID}.R;
        R_C_l = state_vector{PID}.R*extrinsic(1:3,1:3);
        Tik = P_C_k-P_C_i;
        Til = P_C_l-P_C_i;
        Dik = norm(Tik);
        DotP = Xj'*Tik;
