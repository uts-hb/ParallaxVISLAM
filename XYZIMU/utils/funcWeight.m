function [W] = funcWeight(uv,PVector,pre_imu,Dataset)

W_ID1 = [];
W_ID2 = [];
VAL = [];

W_BA_ID1 = [1:size(uv,1)];
W_BA_ID2 = [1:size(uv,1)];
val_BA = [ones(1,size(uv,1))];
...
for i = 1 : size(PVector.Pos,2)-1
    W_IMU_ID1 = [W_IMU_ID1, repelem([size(uv,1)+15*(i-1)+1:size(uv,1)+15*(i-1)+9],9)];
    W_IMU_ID2 = [W_IMU_ID2,repmat([size(uv,1)+15*(i-1)+1:size(uv,1)+15*(i-1)+9],[1,9])];
...
W_B_ID2 = [];
for i = 1 : size(PVector.Pos,2)-1
    W_B_ID1 = [W_B_ID1, size(uv,1)+15*(i-1)+10:size(uv,1)+15*(i-1)+15];
    W_B_ID2 = [W_B_ID2, size(uv,1)+15*(i-1)+10:size(uv,1)+15*(i-1)+15];
