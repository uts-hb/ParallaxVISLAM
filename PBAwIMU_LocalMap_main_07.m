...
GTName = strcat('DataPrepareBA/ORB_2808_07/','GT_P0_PA.mat');
load(GTName); 
...
file = strcat('DataPrepareBA/ORB_2808_07/Image',int2str(start_ImageNum));
load(file);
Image_T_start = [eul2rotm(Image(1,1:3)),Image(1,4:6)'; 0 0 0 1]; 
for i=start_ImageNum:end_ImageNum;
    file = strcat('DataPrepareBA/ORB_2808_07/Image',int2str(i));
	load(file);
	fprintf('%s\n', file);
...
file = strcat('local_map_07_',int2str(start_ImageNum),'_',int2str(end_ImageNum));
diary(file);
%% Least Squares GN
tic
...
% dlmwrite('PBA_IMU_traj_2808_local_07.txt',PBA_so3_traj,'delimiter',' ')
% dlmwrite('rpg_PBA_IMU_traj_2808_local_07.txt',rpg_PBA_so3_traj,'delimiter',' ')
