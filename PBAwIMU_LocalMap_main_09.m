...
load imu_pre_2808_09.mat 
load trajectory_2808_09.mat
...
GTName = strcat('DataPrepareBA/ORB_2808_09/','GT_P0_PA.mat');
load(GTName); 
...
file = strcat('DataPrepareBA/ORB_2808_09/Image',int2str(start_ImageNum));
load(file);
Image_T_start = [eul2rotm(Image(1,1:3)),Image(1,4:6)'; 0 0 0 1]; 
for i=start_ImageNum:end_ImageNum;
    file = strcat('DataPrepareBA/ORB_2808_09/Image',int2str(i));
	load(file);
	fprintf('%s\n', file);
...
%% Saving original feature ID 
% local_feature_id = Feature;
% feature_id_observed_in_first_pose = []; 
% feature_id_observed_in_last_pose = []; 
% for i = 1 : size(local_feature_id,1)
%     if local_feature_id(i,4) == 0
%         local_feature_id(i,:) = 0;
%     end 
% end 
% local_feature_id( all(~local_feature_id,2), : ) = [];
% 
% 
% obs_feat ={}; 
% for i = 1 : size(local_feature_id,1)
%     temp = [];
%     for j = 1 : local_feature_id(i,5)
%         temp = [temp,local_feature_id(i,6 + 3*(j-1))];
%     end 
%     obs_feat{end+1} = temp;
% end 
% 
% for i = 1 : size(obs_feat,2)
%     if ~isempty(find(obs_feat{1,i} == 1))
%         feature_id_observed_in_first_pose = [feature_id_observed_in_first_pose;local_feature_id(i,1)];
%     end 
%     if ~isempty(find(obs_feat{1,i} == end_ImageNum-(start_ImageNum-1)))
%         feature_id_observed_in_last_pose = [feature_id_observed_in_last_pose;local_feature_id(i,1)];
%     end 
% end 
...
%% First Column is global feature ID and second column are local feature ID
% [C,ia,ib]=intersect(feature_id_observed_in_first_pose(:,1),feature_id_change(:,1));
% feature_id_observed_in_first_pose = [feature_id_observed_in_first_pose,feature_id_change(ib,2)];
% [C,ia,ib]=intersect(feature_id_observed_in_last_pose(:,1),feature_id_change(:,1));
% feature_id_observed_in_last_pose = [feature_id_observed_in_last_pose,feature_id_change(ib,2)];
...
file = strcat('local_map_09_',int2str(start_ImageNum),'_',int2str(end_ImageNum));
diary(file);
%% Least Squares GN
tic
[PVector,Reason,Info,objFun,errorPBA,errorIMU] = FuncLeastSquares_w_IMU(xVector,PVector,Feature,K,CAM_2_IMU,pre);
toc
diary off;
% diary LM; 
%% Levenberg-Marquardt Iteration SBA
    [PVector,Reason,Info,objFun,errorPBA,errorIMU] = FuncLeastSquaresLMSBA_w_imu(xVector,PVector,Feature,K,CAM_2_IMU,pre);
% diary off;
...
% dlmwrite('PBA_IMU_traj_2808_local_09.txt',PBA_so3_traj,'delimiter',' ')
% dlmwrite('rpg_PBA_IMU_traj_2808_local_09.txt',rpg_PBA_so3_traj,'delimiter',' ')
